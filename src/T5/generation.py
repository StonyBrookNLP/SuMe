...
parser.add_argument("--seeds_file",type=str, default="../data/test.txt")
...
parser.add_argument("--topp", type=float, default=0)
...
def prepare_data(filename,input_len,f):
    ...
    for sent in all_samples:
        ...
        f.write(tokenizer.decode(used_input, skip_special_tokens=False))
...
for batch_idx in line_tqdm:
        ...
        if args.generation_type2 == "greedy":
            ...
            greedy_output = model.generate(input_ids = input_ids, 
                                ...
                                temperature=0.7)
            decoded = tokenizer.batch_decode(greedy_output, skip_special_tokens=False)
            for go in decoded:
